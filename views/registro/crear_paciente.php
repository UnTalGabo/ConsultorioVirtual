...
<body>
    ...
    <div class="main-container">
        <div class="card p-4 p-md-5">
            <h2 class="text-center mb-4 fw-bold">
                ...
                Nuevo Paciente - Información Básica
            </h2>
            <?php if (isset($_GET['error']) && $_GET['error'] === 'existe'): ?>
                <div class="alert alert-danger text-center">
                    <strong>¡Error!</strong> Ya existe un paciente con ese número de empleado.
                </div>
            <?php endif; ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const idInput = document.querySelector('input[name="id_empleado"]:not([type="hidden"])');
            const form = idInput.closest('form');
            let aviso = null;
            let lastChecked = '';

            function validarIdEmpleado() {
                const id = idInput.value.trim();
                if (!id || id === lastChecked) return;
                lastChecked = id;

                fetch('../../php/registro/validar_id_empleado.php?id_empleado=' + encodeURIComponent(id))
                    .then(res => res.json())
                    .then(data => {
                        // Elimina aviso anterior
                        if (aviso) aviso.remove();
                        if (data.existe) {
                            aviso = document.createElement('div');
                            aviso.className = 'alert alert-danger mt-2';
                            aviso.innerHTML = '<strong>¡Error!</strong> Ya existe un paciente con ese número de empleado.';
                            idInput.parentNode.appendChild(aviso);
            ...
            idInput.addEventListener('input', function() {
                lastChecked = ''; // Fuerza la validación en cada cambio
                validarIdEmpleado();
            });

            // Opcional: evitar enviar el formulario si el id ya existe
